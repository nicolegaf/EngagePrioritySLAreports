...
function isIrrelevant(row) {
  const s = (row["Label"] || row["Labels"] || "").toLowerCase();
  return s.includes("irrelevant") || s.includes("cc_resolved") || s.includes("cc_duplicate message") || s.includes("cc_on hold");
}
...
function calcMetrics(rows) {
  ...
  // replyMap: parentId → rows where Parent comment ID = parentId (used for Comment threading)
  const replyMap = {};
  for (const row of rows) {
    const pid = row["Parent comment ID"];
    if (!pid) continue;
    if (!replyMap[pid]) replyMap[pid] = [];
    replyMap[pid].push(row);
  }

  const responses = [];

  // Pass 1: Comments — each is an independent thread; agent reply matched by Parent comment ID = customer Conversation item ID
  for (const row of rows) {
    if (!isCustomerMsg(row)) continue;
    if ((row["Content type"] || "").toLowerCase() !== "comment") continue;
    const customerTime = parseDate(row["Date created (UTC)"]);
    if (isNaN(customerTime)) continue;
    if (reportMonth && !inReportMonth(customerTime, reportMonth)) continue;
    const { priorities, customerTypes } = detectLabels(row["Label"] || row["Labels"] || "");
    if (priorities.length === 0 || customerTypes.length === 0) continue;

    const itemId = row["Conversation item ID"];
    const agentReplyEntry = (replyMap[itemId] || [])
      .filter(isAgentMsg)
      .map(m => [m, parseDate(m["Date created (UTC)"])])
      .filter(([, t]) => !isNaN(t) && t >= customerTime && (!reportMonth || inAllowedAgentMonth(t, reportMonth)))
      .sort(([, a], [, b]) => a - b)[0];
    if (!agentReplyEntry) continue;

    const agentTime = agentReplyEntry[1];
    const convId = row["Conversation ID"];
    const url = row["URL"] || row["Permalink"] || row["Falcon URL"]
  ...
  // Pass 2: DMs / Mentions — group by Conversation ID, existing sequential logic
  const convMap = {};
  for (const row of rows) {
    if ((row["Content type"] || "").toLowerCase() === "comment") continue;
  ...
  const totalConversations = new Set(rows.map(r => r["Conversation ID"]).filter(Boolean)).size;
  const reportMonthLabel = reportMonth
    ...
    : null;
  return { responses, totalConversations, totalMessages: rows.length, reportMonthLabel };
}
...
function calcJacksData(rows, reportMonth) {
  ...
  for (const row of rows) {
    ...
    if (isNaN(customerTime)) continue;
    if (reportMonth && !inReportMonth(customerTime, reportMonth)) continue;
    const { customerTypes, priorities } = detectLabels(row["Label"] || row["Labels"] || "");
    const convId = row["Conversation ID"];
    const url = row["URL"] || row["Permalink"] || row["Falcon URL"]
      || `https://app.falcon.io/#/engage/${convId}/${convId}`;
